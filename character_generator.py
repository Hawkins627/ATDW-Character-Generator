...
if st.button("📜 Generate Character Sheet"):
    ...
    fields = {
        ...
        "Background": str(bg["background"].split(" - ")[0]) if bg is not None else "",
        "Earn your place": str(ep["earn_place"]) if ep is not None else "",
        "Life changing event": str(le["life_event"]) if le is not None else "",
        "Drive": str(drive_name or ""),
        "Personality": "",  # intentionally blank

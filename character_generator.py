...
# Rulebook: start at 8 in each attribute, distribute 12 points total, max attribute value 18.
BASE_ATTR = 8
ATTR_POINT_BUDGET = 12
ATTR_MAX = 18
ATTR_ADD_MAX = ATTR_MAX - BASE_ATTR  # 10
...
# NOTE: sliders represent *additional* points above the base 8.
attrs = {}
...
for i, label in enumerate(attribute_info.keys()):
    with attr_cols[i]:
        val = st.slider(label, 0, ATTR_ADD_MAX, 0, key=f"attr_{label}", help=attribute_info[label])
        attrs[label] = val
        total_points += val
        st.caption(f"Total: {BASE_ATTR + val}")

# Final attribute values (what should appear on the PDF)
attrs_total = {k: BASE_ATTR + v for k, v in attrs.items()}

if total_points > ATTR_POINT_BUDGET:
    st.error(f"⚠️ Total exceeds {ATTR_POINT_BUDGET}! Currently: {total_points}")
else:
    st.info(f"Attribute points used: {total_points}/{ATTR_POINT_BUDGET}")
...
if st.button("📜 Generate Character Sheet"):
    # Validate primary attribute point budget before generating the PDF
    if total_points > ATTR_POINT_BUDGET:
        st.error(f"⚠️ You have spent {total_points} attribute points, but you may only spend {ATTR_POINT_BUDGET}.")
        st.stop()
    ...
    fields = {
        "tf_name": str(name or ""),
        "tf_pa_str": str(attrs_total["STR"]),
        "tf_pa_dex": str(attrs_total["DEX"]),
        "tf_pa_con": str(attrs_total["CON"]),
        "tf_pa_wil": str(attrs_total["WIL"]),
        "tf_pa_int": str(attrs_total["INT"]),
        "tf_pa_cha": str(attrs_total["CHA"]),
